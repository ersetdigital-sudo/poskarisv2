  ...
  return (
    <div>
      <div style={{ marginBottom:24, display:'flex', alignItems:'center', gap:12 }}>
        ...
        <div>
          <h1 className="text-h1" style={{ fontSize:20, marginBottom:2 }}>Jual Unit Laptop</h1>
          <p style={{ fontSize:13, color:'var(--mute)' }}>Catat penjualan unit laptop</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth:640 }}>
        <div className="card" style={{ padding:24 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            {selectedUnit && (
              <div style={{ padding:'12px 14px', background:'var(--surface-muted)', borderRadius:8 }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, fontSize:13 }}>
                  <div><span style={{ color:'var(--mute)' }}>Unit: </span><span style={{ fontWeight:500, color:'var(--ink)' }}>{selectedUnit.brand} {selectedUnit.model}</span></div>
                  <div><span style={{ color:'var(--mute)' }}>Kondisi: </span><span style={{ fontWeight:500, color:'var(--ink)', textTransform:'capitalize' }}>{selectedUnit.condition}</span></div>
                  <div><span style={{ color:'var(--mute)' }}>Harga Beli: </span><span style={{ fontWeight:500, color:'var(--ink)' }}>{formatRupiah(selectedUnit.buy_price)}</span></div>
                  {selectedUnit.imei_serial && <div><span style={{ color:'var(--mute)' }}>SN: </span><span style={{ fontWeight:500, color:'var(--ink)' }}>{selectedUnit.imei_serial}</span></div>}
                </div>
              </div>
            )}
            <div style={{ borderTop:'1px solid var(--divider)', paddingTop:16 }}>
              <h3 className="text-h3" style={{ marginBottom:10 }}>Data Pembeli</h3>
            ...
            {selectedUnit && (
              <div style={{ padding:'10px 14px', background:'var(--info-bg)', borderRadius:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontSize:13, color:'var(--mute)' }}>Margin</span>
                <span style={{ fontSize:15, fontWeight:600, color:'var(--primary)' }}>{formatRupiah(form.sell_price - selectedUnit.buy_price)}</span>
              </div>
            )}
