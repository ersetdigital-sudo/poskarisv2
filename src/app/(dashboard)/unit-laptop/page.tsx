    ...
    <div>
      <div style={{ marginBottom:24, display:'flex', flexDirection:'column', gap:16, alignItems:'flex-start' }}>
        <div style={{ flex:1 }}>
          ...
          <p className="text-small" style={{ color:'var(--mute)' }}>Kelola stok unit laptop</p>
        </div>
      ...
      <div className="table-wrapper">
        <div style={{ overflowX:'auto' }}>
          <table className="table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign:'center', padding:40, color:'var(--mute)' }}>Belum ada unit laptop</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id}>
                  <td>
                    <p style={{ fontWeight:500, color:'var(--ink)' }}>{p.brand} {p.model}</p>
                    <p style={{ fontSize:12, color:'var(--mute)' }}>{p.sku || '-'}</p>
                  </td>
                  <td style={{ color:'var(--body)', maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.specs || '-'}</td>
                  <td style={{ color:'var(--body)', textTransform:'capitalize' }}>{p.condition || '-'}</td>
