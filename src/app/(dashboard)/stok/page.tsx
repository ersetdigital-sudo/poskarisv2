    ...
    <div>
      ...
      <div style={{ marginBottom: 'var(--space-lg)', display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <p className="text-small" style={{ color: 'var(--color-ink-3)' }}>Kelola stok unit laptop dan sparepart</p>
        </div>
      ...
      <div className="table-wrapper">
        <div style={{ overflowX: 'auto' }}>
          <table className="table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 'var(--space-xl)' }}>
                  <Package size={28} style={{ color: 'var(--color-ink-3)', margin: '0 auto 8px', display: 'block' }} />
                  <p style={{ color: 'var(--color-ink-3)' }}>Belum ada data stok</p>
                </td></tr>
              ) : filtered.map(p => {
                const catName = (p as Product & { categories?: { name: string } }).categories?.name || '-'
                ...
                return (
                  <tr key={p.id} style={{ background: isLow ? 'var(--color-warning-bg)' : undefined }}>
                    <td>
                    ...
                    <td style={{ color: 'var(--color-ink-2)' }}>{catName}</td>
                    <td>
                      <span style={{ fontWeight: 500, color: isLow ? 'var(--color-warning)' : 'var(--color-ink)' }}>{p.quantity}</span>
                      {isLow && <AlertTriangle size={12} style={{ marginLeft: 4, color: 'var(--color-warning)', display: 'inline' }} />}
                    </td>
                    <td style={{ color: 'var(--color-ink-2)' }}>{formatRupiah(p.buy_price)}</td>
                    <td style={{ color: 'var(--color-ink-2)' }}>{formatRupiah(p.sell_price)}</td>
      ...
      {selectedProduct && (
        <div className="modal-overlay" onClick={() => setSelectedProduct(null)}>
          <div className="modal" style={{ maxWidth: 440, padding: 'var(--space-lg)' }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-ink-3)', marginBottom: 'var(--space-lg)' }}>{selectedProduct.name} — Stok: <strong style={{ color: 'var(--color-ink)' }}>{selectedProduct.quantity}</strong></p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2xs)', maxHeight: 300, overflowY: 'auto' }}>
              {movements.filter(m => m.product_id === selectedProduct.id).map(m => (
                <div key={m.id} style={{
                  display: 'flex', alignItems: 'center', gap: 'var(--space-xs)', padding: 'var(--space-xs)',
                  borderRadius: 'var(--radius-md)', border: '1px solid var(--color-rule)',
                }}>
                  {m.type === 'masuk' ? <ArrowDown size={16} color="var(--color-success)" /> : <ArrowUp size={16} color="var(--color-danger)" />}
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 'var(--text-body)', fontWeight: 500, color: 'var(--color-ink)' }}>{m.type === 'masuk' ? '+' : '-'}{m.quantity}</p>
                    <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-ink-3)' }}>{m.notes || m.reference_type}</p>
                  </div>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-ink-3)' }}>{new Date(m.created_at).toLocaleDateString('id-ID')}</span>
                </div>
              ))}
              {movements.filter(m => m.product_id === selectedProduct.id).length === 0 && (
                <p style={{ padding: 'var(--space-lg)', textAlign: 'center', color: 'var(--color-ink-3)' }}>Belum ada mutasi</p>
              )}
            </div>
          </div>
        </div>
